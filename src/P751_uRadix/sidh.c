...
static void fp2_encode(f2elm_t x, unsigned char *enc)
...
static void fp2_decode(const unsigned char *enc, f2elm_t y)
...
{// Alice's ephemeral public key generation
 ...
 // Traverse tree
    ...
    fp2mul_uRadix(phiP->X, phiP->Z, phiP->X);
    fp2mul_uRadix(phiQ->X, phiQ->Z, phiQ->X);
    fp2mul_uRadix(phiR->X, phiR->Z, phiR->X);
...
{// Alice's ephemeral shared secret computation
 ...
 // Output: a shared secret SharedSecretA that consists of one element in GF(p^2) encoded by removing leading 0 bytes.  
    ...
    f2elm_t A24plus = {0}, C24 = {0}, A = {0}, temp;
    unsigned int i, row, m, index = 0, pts_index[MAX_INT_POINTS_ALICE], npts = 0, ii = 0;
 ...
 // Initialize constants: A24plus = A+2C, C24 = 4C, where C=1
    ...
    fp_add_new((digit_t*)&uRadix_one, (digit_t*)&uRadix_one, C24[0], NWORDS_FIELD);
    fp2add(A, C24, A24plus);
    fp_add_new(C24[0], C24[0], C24[0], NWORDS_FIELD);
...
{// Bob's ephemeral shared secret computation
 ...
 // Initialize constants: A24plus = A+2C, A24minus = A-2C, where C=1
    get_A(PKB[0], PKB[1], PKB[2], A);
    fp_add_new((digit_t*)&uRadix_one, (digit_t*)&uRadix_one, A24minus[0], NWORDS_FIELD);
