...
void xDBL(point_proj_t P, point_proj_t Q, f2elm_t A24plus, f2elm_t C24)
{// Coordinate doubling in projective coordinates (X:Z).
 ...
 // Output: projective x-coordinates Q = 2*P = (X2:Z2) in the new data representation based on the unconventional radix.
    ...
    fp2sqr_uRadix(t0, t0);                          // t0 = (X1-Z1)^2 
    fp2sqr_uRadix(t1, t1);                          // t1 = (X1+Z1)^2 
    fp2mul_uRadix(C24, t0, Q->Z);                   // Z2 = C24*(X1-Z1)^2   
    fp2mul_uRadix(t1, Q->Z, Q->X);                  // X2 = C24*(X1-Z1)^2*(X1+Z1)^2
    fp2sub(t1, t0, t1);                             // t1 = (X1+Z1)^2-(X1-Z1)^2 
    fp2mul_uRadix(A24plus, t1, t0);                 // t0 = A24plus*[(X1+Z1)^2-(X1-Z1)^2]
    fp2add(Q->Z, t0, Q->Z);                         // Z2 = A24plus*[(X1+Z1)^2-(X1-Z1)^2] + C24*(X1-Z1)^2
    fp2mul_uRadix(Q->Z, t1, Q->Z);                  // Z2 = [A24plus*[(X1+Z1)^2-(X1-Z1)^2] + C24*(X1-Z1)^2]*[(X1+Z1)^2-(X1-Z1)^2]
}

void xDBLe(point_proj_t P, point_proj_t Q, f2elm_t A24plus, f2elm_t C24, const int e)
...
void get_2_isog(point_proj_t P, f2elm_t A, f2elm_t C)
{// Computes the corresponding 2-isogeny of a projective point (X2:Z2) of order 2.
 ...
 // Output: the 2-isogenous Montgomery curve with projective coefficients A/C.
    ...
    fp2sqr_uRadix(P->X, A);                         // A = X2^2
    fp2sqr_uRadix(P->Z, C);                         // C = Z2^2
...
{// Evaluates the isogeny at the point (X:Z) in the domain of the isogeny, given a 2-isogeny phi.
 ...
 // Output: the projective point P = phi(P) = (X:Z) in the new data representation based on the unconventional radix. 
    ...
    fp2mul_uRadix(t0, t3, t0);                      // t0 = (X2+Z2)*(X-Z)
    fp2mul_uRadix(t1, t2, t1);                      // t1 = (X2-Z2)*(X+Z)
    fp2add(t0, t1, t2);                             // t2 = (X2+Z2)*(X-Z) + (X2-Z2)*(X+Z)
    fp2sub(t0, t1, t3);                             // t3 = (X2+Z2)*(X-Z) - (X2-Z2)*(X+Z)
    fp2mul_uRadix(P->X, t2, P->X);                  // Xfinal
    fp2mul_uRadix(P->Z, t3, P->Z);                  // Zfinal
}

#endif

void get_4_isog(point_proj_t P, f2elm_t A24plus, f2elm_t C24, f2elm_t* coeff)
{// Computes the corresponding 4-isogeny of a projective point (X4:Z4) of order 4.
 ...
 //         that are used to evaluate the isogeny at a point in eval_4_isog().
    ...
    fp2sqr_uRadix(P->Z, coeff[0]);                  // coeff[0] = Z4^2
    fp2add(coeff[0], coeff[0], coeff[0]);           // coeff[0] = 2*Z4^2
    fp2sqr_uRadix(coeff[0], C24);                   // C24 = 4*Z4^4
    fp2add(coeff[0], coeff[0], coeff[0]);           // coeff[0] = 4*Z4^2
    fp2sqr_uRadix(P->X, A24plus);                   // A24plus = X4^2
    fp2add(A24plus, A24plus, A24plus);              // A24plus = 2*X4^2
    fp2sqr_uRadix(A24plus, A24plus);                // A24plus = 4*X4^4
}
...
{// Evaluates the isogeny at the point (X:Z) in the domain of the isogeny, given a 4-isogeny phi defined 
 ...
 // Output: the projective point P = phi(P) = (X:Z) in the new data representation based on the unconventional radix. 
    ...
    fp2mul_uRadix(t0, coeff[1], P->X);              // X = (X+Z)*coeff[1]
    fp2mul_uRadix(t1, coeff[2], P->Z);              // Z = (X-Z)*coeff[2]
    fp2mul_uRadix(t0, t1, t0);                      // t0 = (X+Z)*(X-Z)
    fp2mul_uRadix(t0, coeff[0], t0);                // t0 = coeff[0]*(X+Z)*(X-Z)
    fp2add(P->X, P->Z, t1);                         // t1 = (X-Z)*coeff[2] + (X+Z)*coeff[1]
    fp2sub(P->X, P->Z, P->Z);                       // Z = (X-Z)*coeff[2] - (X+Z)*coeff[1]  ???
    fp2sqr_uRadix(t1, t1);                          // t1 = [(X-Z)*coeff[2] + (X+Z)*coeff[1]]^2
    fp2sqr_uRadix(P->Z, P->Z);                      // Z = [(X-Z)*coeff[2] - (X+Z)*coeff[1]]^2
    ...
    fp2mul_uRadix(P->X, t1, P->X);                  // Xfinal
    fp2mul_uRadix(P->Z, t0, P->Z);                  // Zfinal
}


void xTPL(point_proj_t P, point_proj_t Q, f2elm_t A24minus, f2elm_t A24plus)              
{// Coordinate Tripling in projective coordinates (X:Z).
 ...
 // Output: projective x-coordinates Q = 3*P = (X3:Z3) in the new data representation based on the unconventional radix.
    ...
    fp2sqr_uRadix(t0, t2);                          // t2 = (X-Z)^2           
    fp2add(P->X, P->Z, t1);                         // t1 = X+Z 
    fp2sqr_uRadix(t1, t3);                          // t3 = (X+Z)^2
    fp2add(t0, t1, t4);                             // t4 = 2*X
    fp2sub(t1, t0, t0);                             // t0 = 2*Z 
    fp2sqr_uRadix(t4, t1);                          // t1 = 4*X^2
    fp2sub(t1, t3, t1);                             // t1 = 4*X^2 - (X+Z)^2 
    fp2sub(t1, t2, t1);                             // t1 = 4*X^2 - (X+Z)^2 - (X-Z)^2
    fp2mul_uRadix(t3, A24plus, t5);                 // t5 = A24plus*(X+Z)^2 
    fp2mul_uRadix(t3, t5, t3);                      // t3 = A24plus*(X+Z)^3   ???
    fp2mul_uRadix(A24minus, t2, t6);                // t6 = A24minus*(X-Z)^2
    fp2mul_uRadix(t2, t6, t2);                      // t2 = A24minus*(X-Z)^3  ???
    fp2sub(t2, t3, t3);                             // t3 = A24minus*(X-Z)^3 - coeff*(X+Z)^3 ???
    fp2sub(t5, t6, t2);                             // t2 = A24plus*(X+Z)^2 - A24minus*(X-Z)^2
    fp2mul_uRadix(t1, t2, t1);                      // t1 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2]
    fp2add(t3, t1, t2);                             // t2 = [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2] + A24minus*(X-Z)^3 - coeff*(X+Z)^3
    fp2sqr_uRadix(t2, t2);                          // t2 = t2^2
    fp2mul_uRadix(t4, t2, Q->X);                    // X3 = 2*X*t2
    fp2sub(t3, t1, t1);                             // t1 = A24minus*(X-Z)^3 - A24plus*(X+Z)^3 - [4*X^2 - (X+Z)^2 - (X-Z)^2]*[A24plus*(X+Z)^2 - A24minus*(X-Z)^2]
    fp2sqr_uRadix(t1, t1);                          // t1 = t1^2
    fp2mul_uRadix(t0, t1, Q->Z);                    // Z3 = 2*Z*t1
}


void xTPLe(point_proj_t P, point_proj_t Q, f2elm_t A24minus, f2elm_t A24plus, const int e)
...
void get_3_isog(point_proj_t P, f2elm_t A24minus, f2elm_t A24plus, f2elm_t* coeff)
{// Computes the corresponding 3-isogeny of a projective point (X3:Z3) of order 3.
 ...
 // Output: the 3-isogenous Montgomery curve with projective coefficient A/C. 
    ...
    fp2mul_uRadix(t2, t4, A24minus);                // A24minus = [4*X^2 - (X-Z)^2]*[8*X^2 - (X+Z)^2 + 2*(X-Z)^2]
    fp2add(t1, t2, t4);                             // t4 = 4*X^2 + (X+Z)^2 - (X-Z)^2
    fp2add(t4, t4, t4);                             // t4 = 2(4*X^2 + (X+Z)^2 - (X-Z)^2) 
    fp2add(t0, t4, t4);                             // t4 = 8*X^2 + 2*(X+Z)^2 - (X-Z)^2
    fp2mul_uRadix(t3, t4, A24plus);                 // A24plus = [4*X^2 - (X+Z)^2]*[8*X^2 + 2*(X+Z)^2 - (X-Z)^2]
}
...
{// Computes the 3-isogeny R=phi(X:Z), given projective point (X3:Z3) of order 3 on a Montgomery curve and 
 ...
 // Output: the projective point Q <- phi(Q) = (X3:Z3) in the new data representation based on the unconventional radix. 
    ...
    fp2mul_uRadix(t0, coeff[0], t0);              // t0 = coeff0*(X+Z)
    fp2mul_uRadix(t1, coeff[1], t1);              // t1 = coeff1*(X-Z)
    fp2add(t0, t1, t2);                           // t2 = coeff0*(X+Z) + coeff1*(X-Z)
    fp2sub(t1, t0, t0);                           // t0 = coeff1*(X-Z) - coeff0*(X+Z)
    fp2sqr_uRadix(t2, t2);                        // t2 = [coeff0*(X+Z) + coeff1*(X-Z)]^2
    fp2sqr_uRadix(t0, t0);                        // t0 = [coeff1*(X-Z) - coeff0*(X+Z)]^2
    fp2mul_uRadix(Q->X, t2, Q->X);                // X3final = X*[coeff0*(X+Z) + coeff1*(X-Z)]^2        
    fp2mul_uRadix(Q->Z, t0, Q->Z);                // Z3final = Z*[coeff1*(X-Z) - coeff0*(X+Z)]^2
}
...
{// 3-way simultaneous inversion
 ...
 // Output: 1/z1,1/z2,1/z3 (override inputs).
    ...
    fp2mul_uRadix(z1, z2, t0);                      // t0 = z1*z2
    fp2mul_uRadix(z3, t0, t1);                      // t1 = z1*z2*z3
    fp2inv_uRadix(t1);                              // t1 = 1/(z1*z2*z3)
    fp2mul_uRadix(z3, t1, t2);                      // t2 = 1/(z1*z2) 
    fp2mul_uRadix(t2, z2, t3);                      // t3 = 1/z1
    fp2mul_uRadix(t2, z1, z2);                      // z2 = 1/z2
    fp2mul_uRadix(t0, t1, z3);                      // z3 = 1/z3
    fp2copy(t3, z1);                                // z1 = 1/z1
}


void get_A(f2elm_t xP, f2elm_t xQ, f2elm_t xR, f2elm_t A)
{// Given the x-coordinates of P, Q, and R, returns the value A corresponding to the Montgomery curve E_A: y^2=x^3+A*x^2+x such that R=Q-P on E_A.
 ...
 // Output: the coefficient A corresponding to the curve E_A: y^2=x^3+A*x^2+x.
    ...
    fp2mul_uRadix(xP, xQ, t0);                    // t0 = xP*xQ
    fp2mul_uRadix(xR, t1, A);                     // A = xR*t1
    fp2add(t0, A, A);                             // A = A+t0
    fp2mul_uRadix(t0, xR, t0);                    // t0 = t0*xR
    fp2sub(A, one, A);                            // A = A-1
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2add(t1, xR, t1);                           // t1 = t1+xR
    fp2add(t0, t0, t0);                           // t0 = t0+t0
    fp2sqr_uRadix(A, A);                          // A = A^2
    fp2inv_uRadix(t0);                            // t0 = 1/t0
    fp2mul_uRadix(A, t0, A);                      // A = A*t0
...
{// Computes the j-invariant of a Montgomery curve with projective constant.
 ...
 // Output: j=256*(A^2-3*C^2)^3/(C^4*(A^2-4*C^2)), which is the j-invariant of the Montgomery curve B*y^2=x^3+(A/C)*x^2+x or (equivalently) j-invariant of B'*y^2=C*x^3+A*x^2+C*x.
    ...
    fp2sqr_uRadix(A, jinv);                         // jinv = A^2        
    fp2sqr_uRadix(C, t1);                           // t1 = C^2
    fp2add(t1, t1, t0);                             // t0 = t1+t1
    fp2sub(jinv, t0, t0);                           // t0 = jinv-t0
    fp2sub(t0, t1, t0);                             // t0 = t0-t1
    fp2sub(t0, t1, jinv);                           // jinv = t0-t1
    fp2sqr_uRadix(t1, t1);                          // t1 = t1^2
    fp2mul_uRadix(jinv, t1, jinv);                  // jinv = jinv*t1
    fp2add(t0, t0, t0);                             // t0 = t0+t0
    fp2add(t0, t0, t0);                             // t0 = t0+t0
    fp2sqr_uRadix(t0, t1);                          // t1 = t0^2
    fp2mul_uRadix(t0, t1, t0);                      // t0 = t0*t1
    fp2add(t0, t0, t0);                             // t0 = t0+t0
    fp2add(t0, t0, t0);                             // t0 = t0+t0
    fp2inv_uRadix(jinv);                            // jinv = 1/jinv 
    fp2mul_uRadix(jinv, t0, jinv);                  // jinv = t0*jinv
}
...
{// Simultaneous doubling and differential addition.
 ...
 // Output: projective points P <- 2*P = (X2P:Z2P) such that x(2P)=X2P/Z2P, and Q <- P+Q = (XQP:ZQP) such that = x(Q+P)=XQP/ZQP, in the new data representation. 
    ...
    fp2sqr_uRadix(t0, P->X);                        // XP = (XP+ZP)^2
    fp2sub(Q->X, Q->Z, t2);                         // t2 = XQ-ZQ
    fp2add(Q->X, Q->Z, Q->X);                       // XQ = XQ+ZQ
    fp2mul_uRadix(t0, t2, t0);                      // t0 = (XP+ZP)*(XQ-ZQ)
    fp2sqr_uRadix(t1, P->Z);                        // ZP = (XP-ZP)^2
    fp2mul_uRadix(t1, Q->X, t1);                    // t1 = (XP-ZP)*(XQ+ZQ)
    fp2sub(P->X, P->Z, t2);                         // t2 = (XP+ZP)^2-(XP-ZP)^2
    fp2mul_uRadix(P->X, P->Z, P->X);                // XP = (XP+ZP)^2*(XP-ZP)^2
    fp2mul_uRadix(t2, A24, Q->X);                   // XQ = A24*[(XP+ZP)^2-(XP-ZP)^2]
    ...
    fp2mul_uRadix(P->Z, t2, P->Z);                  // ZP = [A24*[(XP+ZP)^2-(XP-ZP)^2]+(XP-ZP)^2]*[(XP+ZP)^2-(XP-ZP)^2]
    fp2sqr_uRadix(Q->Z, Q->Z);                      // ZQ = [(XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)]^2
    fp2sqr_uRadix(Q->X, Q->X);                      // XQ = [(XP+ZP)*(XQ-ZQ)+(XP-ZP)*(XQ+ZQ)]^2
    fp2mul_uRadix(Q->Z, xPQ, Q->Z);                 // ZQ = xPQ*[(XP+ZP)*(XQ-ZQ)-(XP-ZP)*(XQ+ZQ)]^2
}
...
static void LADDER3PT(f2elm_t xP, f2elm_t xQ, f2elm_t xPQ, digit_t* m, const unsigned int AliceOrBob, point_proj_t R, f2elm_t A)
{
