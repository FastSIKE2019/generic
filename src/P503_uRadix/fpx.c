...
{// GF(p^2) division by two, c = a/2  in GF(p^2).
 ...
 // Output : c = c0 + c1*i, c = a/2; c0 & c1 in uncoventional radix representation, meet the same requirement with fpmul_new & fpsqr_new
    ...
    fpdiv2_503_new(a[0], c[0]);
    fpdiv2_503_new(a[1], c[1]);
}
...
{// Conversion of a GF(p^2) element from the new data representation based on the unconventional radix to standard representation.
...
{// Conversion of a GF(p^2) element from the standard representation to the new data representation based on the unconventional radix.
